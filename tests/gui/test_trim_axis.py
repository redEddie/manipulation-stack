...
시간 구간으로 자르는 재설계는 아직이다. 그때까지 **거부한다.** 120 Hz 의
27 tick 은 0.225초이고 30 fps 카메라로는 7장이라, 27 을 그대로 적용하면 카메라가
0.9초를 더 잃는다. 잘못 자른 파일을 만드느니 멈추는 쪽이 낫다.

여기서 확인하는 것:
1. 1.3.0 은 예전과 **똑같이** 잘린다 (모든 계열이 함께)
2. 2.0.0 은 거부하고, **파일이 하나도 안 바뀐다** -- 반쯤 자르다 멈추면
   되돌릴 수 없다
"""
import os
import sys
...
def main() -> None:
    ...
    try:
        ...
        msg = str(e)
        assert "axis" in msg, f"이유를 안 밝힌다: {msg}"
    else:
        raise AssertionError(
            "knu-2.0.0 을 잘랐다 -- 축마다 길이가 다르면 카메라가 조용히 "
            "건너뛰어져 액션만 잘린 에피소드가 된다")
    after = shapes(p2)
    assert after == before == (N, N, N, N), (
        f"거부했는데 파일이 바뀌었다 {before} -> {after} -- 반쯤 자르다 "
        "멈추면 되돌릴 수 없다")
    print(f"2. knu-2.0.0: 거부하고 파일은 {after[0]}프레임 그대로 OK")
