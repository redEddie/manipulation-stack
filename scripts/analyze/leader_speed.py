...
수집 워커는 리더 관절속도를 100 ms 창으로 관절별 평균 낸 뒤 그 7개 값의 L2
노름이 2.4 rad/s 를 넘으면 팔을 세우고 에피소드를 폐기한다. 그 2.4 는 낙하
**1건**과 정상 5개(28.9초)에서 나왔다. 표본이 얇으므로, 세션이 쌓이면 이
도구로 다시 봐야 한다.
...
읽는 법. **정상 에피소드의 최고값이 임계에 얼마나 붙었나**를 본다. 1.5 근처에
머물면 여유가 있고, 2.0 을 넘기 시작하면 오검이 임박한 것이다. 오검은 한
