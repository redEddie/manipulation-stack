...
셋(공간 회수·HDF5 업로드·LeRobot 변환)은 하는 일이 같다: **어느 .hdf5 를 고를
것인가**를 정하고, 그 다음에 그 동작의 옵션을 정한다. 그런데 화면은 셋이
따로 자랐다 -- 공간 회수만 체크 목록이고 나머지 둘은 경로를 공백으로 이어 붙인
...
* 공간 회수 -- "빈 공간 320 MB (12%) — 다시 필요"
* 업로드 -- "신규 — 업로드 기록 없음" / "변경 — 09/12 14:02 수정"
* 변환   -- 에피소드 수 (변환은 파일 상태와 무관하게 전부가 대상이다)

공간 회수(repack)는 두 일을 한다 -- 옛 lzf 파일의 이미지 재압축과, 지운
에피소드가 남긴 죽은 공간의 회수. 2026-09-22 부터 수집기는 처음부터 gzip 으로
쓰므로, 새 파일에서 남은 일은 후자다.

**기본 체크가 요점이다.** 어느 파일이 지금 대상인지 조작자가 기억하고 있을
필요가 없다 -- 파일 자신이 답을 갖고 있고(공간 회수 이력·업로드 장부), 그것이
아는 만큼만 미리 체크된다.
"""
