...
**왜 LeRobot 변환용과 겸하지 않는가.** 겸하려면 224 크롭에 에피소드별
``crop_params`` 를 적용하고 ``libsvtav1 crf30 g=2`` 로 맞춰야 하는데, 그건
위의 이유로 큐레이션에 못 쓴다. 게다가 ``crop_params`` 가 에피소드별 attrs 라
나중에 크롭을 바꾸면 캐시가 조용히 틀린 영상이 된다 -- 느린 변환보다 틀린
학습 데이터가 훨씬 나쁘다. 변환 가속은 별건으로 다룬다.
