...
_ROBOT_OBS_FIELDS = {
    ...
    # 2.0.0 이 더한 것은 **구조**(계열마다 자기 시간축)이지 새 로봇 관측이
    # 아니다 -- 로봇이 줘야 하는 값은 1.3.0 과 같다. 카메라가 프레임을 전부
    # 내주는지는 노드가 답할 일이라 여기서 묻지 않는다.
    "knu-2.0.0": FT_OBS_KEYS + (ROBOT_STATE_TIME,),
    # 2.1.0 이 더한 것은 metadata attr(그리퍼)이라 로봇 관측은 2.0.0 과 같다.
    "knu-2.1.0": FT_OBS_KEYS + (ROBOT_STATE_TIME,),
}
